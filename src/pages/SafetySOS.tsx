  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-auto lg:h-full w-full pb-4 lg:pb-0">
      <div className="lg:col-span-8 flex flex-col gap-4 h-auto lg:h-full">
        ...
        <div className="bento-card p-6 md:p-8 rounded-3xl flex flex-col h-auto min-h-[300px] lg:flex-1 bg-white/60 relative overflow-hidden">
      ...
      <div className="lg:col-span-4 bento-card p-6 sm:p-8 rounded-3xl flex flex-col h-auto lg:h-full bg-white/90 relative overflow-hidden">
        <form onSubmit={handleReport} className="flex-1 flex flex-col justify-between h-full relative z-10">
           <div className="space-y-6">
              <h2 className="text-xl sm:text-2xl font-black text-slate-900 border-b border-slate-200 pb-4 sm:pb-6">Detailed Incident Report</h2>
           ...
           <button 
             type="submit"
             disabled={loading}
             className="w-full bg-coral-dark hover:bg-coral-dark/90 text-white px-6 py-4 sm:py-5 rounded-2xl font-black transition-all shadow-xl shadow-coral-dark/20 disabled:opacity-70 flex justify-center items-center h-14 sm:h-16 shrink-0 text-base sm:text-lg uppercase mt-6"
           >
             {loading ? <div className="w-5 h-5 border-4 border-white/30 border-t-white rounded-full animate-spin"></div> : 'Dispatch Intelligence'}
           </button>
        </form>
